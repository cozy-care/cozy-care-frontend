...
export default function Messages() {
    ...
    const chat_id = "e2a5a3ed-a995-4b70-9c42-10da36e0086b"; // Fixed chat ID
    ...
    const sortedMessages = messages.sort((a, b) => new Date(b.sent_at).getTime() - new Date(a.sent_at).getTime());
        ...
        <main className="flex min-h-[calc(100svh-3.5rem)]">
            ...
            <div className="flex flex-col grow max-h-[calc(100svh-3.5rem)] border-r-1 border-gray-300">
                ...
                <div className="w-full h-full flex flex-col-reverse gap-1 px-3 pb-2 overflow-y-scroll">
                    {sortedMessages.map((message) => (
                        message.sender_id === userId ? (
                            <MyChatBox key={message.message_id} text={message.content} date={new Date(message.sent_at).toLocaleTimeString()} />
                        ) : (
                            <TheirChatBox key={message.message_id} text={message.content} date={new Date(message.sent_at).toLocaleTimeString()} />
                        )
                    ))}
                    <div ref={messagesEndRef} /> {/* Scroll to this element when a new message is added */}
                </div>
                ...
                <div className="flex h-[64px] items-center justify-between px-4 gap-4 border-t-1 border-gray-300">
                    ...
                    <Input type="text" placeholder="Type your message" value={messageText} onChange={(e) => setMessageText(e.target.value)} onKeyDown={handleKeyPress} className="grow" />
                    <button type="button" className="hover:!text-blue-500" onClick={handleSendMessage}><Send sx={{ fontSize: 30 }} /></button>
                </div>
            </div>
